...
export class VerifyController {
  ...
  static async verifyWorker(
    ...
    try {
      ...
      const worker = await prisma.workerProfile.findUnique({
        where: { userId: workerId as string },
        include: {
          credentials: {
            where: { revoked: false },
          },
      ...
      const behavioralDNA = await BehavioralDNAService.computeForWorker(worker.userId);

      // Get fraud analysis
      const fraudAnalysis = await FraudDetectionService.analyzeAccessPatterns(worker.userId);
      ...
      if (accessRequest.scopeGranted.includes('credentials')) {
        verifiedData.credentials = worker.credentials
          .filter(cred => accessRequest.scopeGranted.some(scope =>
            scope.includes(cred.type.toLowerCase())
          ))
          .map(cred => ({
            type: cred.type,
            tier: cred.tier,
   ...
   */
  static async getLenderDashboard(
    req: Request,
    res: Response
  ): Promise<void> {
    try {
      ...
      const stats = {
        ...
        activeTokens: accessRequests.filter(r =>
          r.status === 'APPROVED' && r.expiresAt > new Date()
        ).length,
      };
