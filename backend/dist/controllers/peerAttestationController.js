...
class PeerAttestationController {
    static async createAttestation(req, res) {
        try {
            ...
            const [subject, attester] = await Promise.all([
                (0, helpers_1.findWorkerProfile)(subjectId),
                (0, helpers_1.findWorkerProfile)(attesterUserId),
            ]);
            if (!subject || !attester) {
            ...
            const attestation = await (0, helpers_1.createPeerAttestation)({
    ...
    static async getAttestations(req, res) {
        try {
            ...
            const subject = await (0, helpers_1.findWorkerProfile)(workerId);
            if (!subject) {
                res.status(404).json({ success: false, error: 'Worker profile not found' });
                return;
            }
            const attestations = await (0, helpers_1.findAttestationsBySubject)(subject.id);
            const safeAttestations = attestations || [];
            const totalWeight = safeAttestations.reduce((sum, att) => sum + att.weight, 0);
    ...
    static async getGivenAttestations(req, res) {
        try {
            ...
            const attester = await (0, helpers_1.findWorkerProfile)(userId);
            if (!attester) {
                res.status(404).json({ success: false, error: 'Worker profile not found' });
                return;
            }
            const attestations = await (0, helpers_1.findAttestationsByAttester)(attester.id);
    ...
    static async updateAttestation(req, res) {
        try {
            ...
            const { data: attestation, error } = await supabase_1.supabase
            ...
            const { data: updatedAttestation, error: updateError } = await supabase_1.supabase
                .from('peer_attestations')
                .update(updateData)
                .eq('id', attestationId)
                .select()
    ...
    static async deleteAttestation(req, res) {
        try {
            ...
            await (0, helpers_1.deleteFromTable)('peer_attestations', { id: attestationId });
    ...
    static async getAttestationStats(req, res) {
        try {
            ...
            const worker = await (0, helpers_1.findWorkerProfile)(userId);
            if (!worker) {
                res.status(404).json({ success: false, error: 'Worker profile not found' });
                return;
            }
            const [receivedAttestations, givenAttestations] = await Promise.all([
                (0, helpers_1.findAttestationsBySubject)(worker.id),
                (0, helpers_1.findAttestationsByAttester)(worker.id),
            ]);
            ...
            const stats = {
                received: {
                    ...
                    byRelationship: received.reduce((acc, att) => {
                        acc[att.relationship] = (acc[att.relationship] || 0) + 1;
                        return acc;
                    }, {}),
                    averageWeight: received.length > 0
                        ? received.reduce((sum, att) => sum + att.weight, 0) / received.length
                        : 0,
                },
                given: {
                    total: given.length,
                    byRelationship: given.reduce((acc, att) => {
                        acc[att.relationship] = (acc[att.relationship] || 0) + 1;
                        return acc;
                    }, {}),
                },
