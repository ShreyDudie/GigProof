...
class BehavioralDNAService {
    ...
    static computeConsistencyIndex(incomeRecords) {
        if (incomeRecords.length < 2)
            return 50; // Default for new workers
    ...
    static computePlatformDiversification(platforms) {
        ...
        platforms.forEach(p => {
            if (['UBER', 'OLA'].includes(p.platformName))
                categories.add('ride-share');
            if (['SWIGGY', 'ZOMATO'].includes(p.platformName))
                categories.add('delivery');
            if (['UPWORK', 'FIVERR'].includes(p.platformName))
                categories.add('freelance');
            if (['URBAN_COMPANY'].includes(p.platformName))
    ...
    static computeGrowthTrajectory(incomeRecords) {
        if (incomeRecords.length < 6)
            return 50; // Need at least 6 months
        // Group by month
    ...
    static computeDemandResponsiveness(incomeRecords) {
    ...
    static computeSkillAcquisitionRate(credentials) {
    ...
    static computeRecoverySpeed(incomeRecords) {
        if (incomeRecords.length < 6)
            return 50;
        // Group by month
        const monthlyIncome = new Map();
        ...
        for (let i = 1; i < sortedMonths.length; i++) {
            ...
            if (currentIncome < prevIncome * 0.8) { // 20% drop
                ...
                for (let j = i + 1; j < sortedMonths.length; j++) {
                    ...
                    if (recoveryIncome >= prevIncome)
                        break;
                }
                // 1 month = 100, 2 months = 75, 3 = 50, never = 10
                if (recoveryMonths === 1)
                    recoveryScore = Math.min(recoveryScore, 100);
                else if (recoveryMonths === 2)
                    recoveryScore = Math.min(recoveryScore, 75);
                else if (recoveryMonths === 3)
                    recoveryScore = Math.min(recoveryScore, 50);
                else
                    recoveryScore = Math.min(recoveryScore, 10);
            }
        }
        return recoveryScore;
    }
    static computeReputationMomentum(incomeRecords) {
    ...
    static computeOverallScore(signals) {
        return Math.round(signals.consistencyIndex * 0.25 +
...
exports.BehavioralDNAService = BehavioralDNAService;
exports.default = BehavioralDNAService;
//# sourceMappingURL=behavioralDNA.js.map